...
RUN \
   ...
   git clone https://github.com/JustJoshTikTok/rdt-client.git . && \
   cd client && \
   echo "**** Building Code  ****" && \
   npm ci && \
...
# Check Status
HEALTHCHECK --interval=30s --timeout=30s --start-period=30s --retries=3 CMD curl --fail http://localhost:6501 || exit 
